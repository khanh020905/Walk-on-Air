...
const Sale = ({ category: propCategory, setCategory: propSetCategory }) => {
  // If props aren't provided (e.g. direct route), manage state locally
  const [localCategory, setLocalCategory] = useState("All");
  const category = propCategory || localCategory;
  const setCategory = propSetCategory || setLocalCategory;

  const [products, setProducts] = useState([]);
  const [priceRange, setPriceRange] = useState([0, 1000]); // Default wider range
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Fetch products with badge=Sale.
        // We do NOT restrict by mainCategory here, so it shows Men/Women/Kids sales.
        let url = `${import.meta.env.VITE_BACKEND_URL}/api/products?page=${
          currentPage - 1
        }&sizeLimit=12&badge=Sale`;
